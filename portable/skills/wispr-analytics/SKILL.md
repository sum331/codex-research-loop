...
description: >
  Analyze Wispr Flow dictation history for self-reflection, work patterns,
  mental-health/productivity insights, and manage the Wispr Flow dictionary
  (add terms, fix mishears, import/export, suggest improvements). Trigger when
  the user says /wispr-analytics, analyze my dictations, what did I dictate
  today, Wispr reflection, add to Wispr dictionary, improve dictation, export
  Wispr dictionary, or says Wispr分析, 语音输入记录, 口述历史, 听写词典, 纠正误识别.
---
...
The scripts resolve the Wispr Flow SQLite database from `WISPR_FLOW_DB` first,
then common platform paths. Known paths include:
```
~/Library/Application Support/Wispr Flow/flow.sqlite
%APPDATA%\Wispr Flow\flow.sqlite
%LOCALAPPDATA%\WisprFlow\flow.sqlite
```
...
1. Check if Wispr is running: use `pgrep -f "Wispr Flow"` on macOS/Linux or `Get-Process Wispr*` on Windows.
2. If running, ask user to quit first (Cmd+Q on macOS; tray/menu quit on Windows)
