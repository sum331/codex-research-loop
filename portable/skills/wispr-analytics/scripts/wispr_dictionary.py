...
def candidate_db_paths():
    env_path = os.environ.get("WISPR_FLOW_DB")
    if env_path:
        yield _expand_path(env_path)

    home = Path.home()
    yield home / "Library" / "Application Support" / "Wispr Flow" / "flow.sqlite"

    appdata = os.environ.get("APPDATA")
    if appdata:
        yield Path(appdata) / "Wispr Flow" / "flow.sqlite"
        yield Path(appdata) / "WisprFlow" / "flow.sqlite"

    localappdata = os.environ.get("LOCALAPPDATA")
    if localappdata:
        yield Path(localappdata) / "WisprFlow" / "flow.sqlite"
        yield Path(localappdata) / "Wispr Flow" / "flow.sqlite"


def resolve_db_path():
    candidates = list(dict.fromkeys(candidate_db_paths()))
...
def get_db(readonly=True):
    if not DB_PATH.exists():
        checked = "\n  - ".join(str(path) for path in dict.fromkeys(candidate_db_paths()))
        print(
            "Error: Wispr Flow database not found. Set WISPR_FLOW_DB to the "
            "flow.sqlite path or sign in and create dictation history first. Checked:\n"
            f"  - {checked}",
            file=sys.stderr,
        )
        sys.exit(1)
        ...
        if os.name == "nt":
            result = subprocess.run(
                [
                    ...
                    "-NoProfile",
                    "-Command",
                    "Get-Process | Where-Object { $_.ProcessName -like 'Wispr*' } | Select-Object -First 1 -ExpandProperty Id",
                ],
                capture_output=True,
                text=True,
            )
...
def require_wispr_stopped():
    if is_wispr_running():
        quit_hint = "quit it from the tray/menu" if os.name == "nt" else "quit it first (Cmd+Q)"
        print(f"Error: Wispr Flow is running. {quit_hint} to avoid database corruption.", file=sys.stderr)
        sys.exit(1)
...
def restart_wispr():
    ...
    try:
        if os.name == "nt":
            ...
            localappdata = os.environ.get("LOCALAPPDATA")
            if localappdata:
                base = Path(localappdata) / "WisprFlow"
                candidates.append(base / "Wispr Flow.exe")
                candidates.extend(sorted(base.glob("app-*/Wispr Flow.exe"), reverse=True))
            exe = next((path for path in candidates if path.exists()), None)
            if not exe:
                raise FileNotFoundError("Wispr Flow.exe not found")
