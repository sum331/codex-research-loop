...
name: cleanshot
description: >
  Use CleanShot X for screenshots, screen recordings, scrolling captures, OCR,
  text extraction, and image annotation. Trigger when the user asks to take a
  screenshot/capture/recording, capture a window or area, do OCR, extract text
  from the screen/image, annotate an image, or says 截图, 录屏, 滚动截图, OCR, 提取文字.
allowed-tools: [Bash]
---
